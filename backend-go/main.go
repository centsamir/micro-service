...
func GormConnectToMySQLDatabase(user, password, dbname string, port int) (*gorm.DB, error) {
	var dsn string
	if dbname == "nil" {
		dsn = fmt.Sprintf("%s:%s@tcp(micro-service-mybddapp-1:%d)/?charset=utf8mb4&parseTime=True&loc=Local", user, password, port)
	} else {
		dsn = fmt.Sprintf("%s:%s@tcp(micro-service-mybddapp-1:%d)/%s?charset=utf8mb4&parseTime=True&loc=Local", user, password, port, dbname)
	}
